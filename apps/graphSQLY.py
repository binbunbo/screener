...
def asset(dat):
    ...
    asset_bar5 = go.Bar(y=statsY.loc[:, "cip"], x=statsY.loc[:, "year"], name="XDCB",
                        marker=dict(color=('goldenrod')))
    asset_bar6 = go.Bar(y=statsY.loc[:, "other_asset"], x=statsY.loc[:, "year"], name="Khác",
                        marker=dict(color=('darkgray')))
    asset_bar7 = go.Scatter(y=statsY.loc[:, "ca/ta"], x=statsY.loc[:, "year"], yaxis='y2',
                            name="TS ngắn hạn/Tổng tài sản", line=dict(color='deeppink', width=3, dash='dot'),
                            mode='lines')
    asset_bar8 = go.Scatter(y=statsY.loc[:, "mc"], x=statsY.loc[:, "year"], yaxis='y', name="Vốn hóa",
